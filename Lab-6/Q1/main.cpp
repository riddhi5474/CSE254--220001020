...
void toposort(vector<vector<int>> &g,vector<int> &ans,vector<bool> & vis,vector<int> &indegree){
    //insert all nodes with 0 indegree in the queue
    //code handles disconnected components as well
    int V=indegree.size();
    queue<int> q;
    for(int i=0;i<V;i++){
        if(indegree[i]==0){
...
int main(){
    ...
    if(ans.size()!=V){
        cout<<"this is not a directed acyclic graph"<<endl;
    }else{
        for(int node:ans){
            cout<<node<<" ";
        }
...
//output- this is not a directed acyclic graph